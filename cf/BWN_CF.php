...
{
$sql="select * from `qqreg` where `qq`='".$_GET['id']."'";//id是通过传递过来的。
...
{       if($_GET['id']>100 && $_GET['id']<10000000000000)//如果小于位数说明是非法操作！
   //不许字母进行插入，因此要进行匹配
   {
               	 //否则就插入条目
 $qq=$_GET['id'];
...
if($rs['hits']>15 && $rs['contents']!=15)//限制只能使用15次
             {
echo $rs['hits']."情迁火线";
	//$sqlhit="update `qqreg` set shuoming='autoadd_noreg_qq694886526超过使用限制，请联系管理' where `qq`='".$_GET['id']."'";
